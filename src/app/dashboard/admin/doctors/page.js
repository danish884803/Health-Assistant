      ...
      <main className="pt-28 pb-20">
        <div className="max-w-7xl mx-auto px-6 space-y-6">
          ...
          <div className="flex justify-between items-center">
            ...
            <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-xl">
              <Plus size={18} /> Add Doctor
            </button>
          </div>
              ...
              <tbody>
                {doctors.map(d => (
                  <tr key={d._id} className="border-t">
                    ...
                    <td className="px-6 py-4">{d.department}</td>
                    <td className="px-6 py-4 flex justify-end gap-3">
                      <button onClick={() => openEdit(d)}><Pencil size={16} /></button>
                      <button onClick={() => deleteDoctor(d._id)}><Trash2 size={16} className="text-red-600" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-3xl p-8 w-full max-w-lg space-y-4">
            ...
            <h2 className="font-bold text-xl">{editingDoctor ? 'Edit Doctor' : 'Add Doctor'}</h2>

            {['name','email','password','department','clinic'].map(f => (
              <input key={f} className="w-full border rounded-xl p-3"
                placeholder={f}
                type={f === 'password' ? 'password' : 'text'}
                value={form[f]}
                onChange={e => setForm({ ...form, [f]: e.target.value })}
            ...
            <select
              ...
              onChange={e => {
                ...
                setForm({
                  ...form,
                  roomId: r?._id,
                  room: r?.roomCode || r?.name,
                });
              }}
            >
              <option value="">Select Room (Map)</option>
              {rooms.map(r => (
                <option key={r._id} value={r._id}>
                  {r.name} — {r.floorId?.name}
                </option>
              ))}
            </select>

            <button onClick={saveDoctor} className="w-full bg-teal-600 text-white py-3 rounded-xl font-bold">
              Save Doctor
            </button>
