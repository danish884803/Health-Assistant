...
export default function AdminDashboard() {
    ...
    async function fetchStats() {
      try {
        ...
        const [patientsData, doctorsData, appointmentsData] = await Promise.all([
          patientsRes.ok ? patientsRes.json() : null,
          doctorsRes.ok ? doctorsRes.json() : null,
        ...
        setCounts({ patients: '—', doctors: '—', appointments: '—' });
      } finally {
  ...
  const stats = [
    {
      label: 'Total Patients',
      value: statsLoading ? '...' : counts.patients,
      icon: <Users size={22} />,
    },
    {
      label: 'Doctors',
      value: statsLoading ? '...' : counts.doctors,
      icon: <UserCog size={22} />,
    },
    {
      label: 'Appointments',
      value: statsLoading ? '...' : counts.appointments,
      icon: <CalendarClock size={22} />,
    },
    {
      label: 'System Health',
