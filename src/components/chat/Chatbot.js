    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          language: /[\u0600-\u06FF]/.test(userMsg.content)
            ? 'ar'
            : 'en'
        }),
      });
      ...
      if (data.intent === 'FIND_DOCTOR' && data.department) {
        ...
        const r = await fetch(
          `/api/doctor?department=${encodeURIComponent(
            data.department
          )}`
        );
        ...
        if (!d.doctors || d.doctors.length === 0) {
          ...
          pushAssistant(
            `🩺 Available doctors in ${data.department}:\n\n${list}`
          );
        }
      ...
      if (data.intent === 'NAVIGATE' && data.roomCode) {
        ...
        pushAssistant(
          `📍 Taking you to ${data.roomCode.replace('-', ' ')}`
        );
      ...
      <AnimatePresence>
        ...
        {open && (
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 20, scale: 0.96 }}
            className="fixed bottom-20 right-4 left-4 sm:left-auto sm:right-6 sm:w-[380px] max-h-[70vh] sm:max-h-[560px] bg-white rounded-[26px] shadow-2xl z-50 flex flex-col overflow-hidden"
          >
                ...
                >
                  ...
                  <div className="flex items-end gap-2 max-w-[90%] sm:max-w-[85%]">
                    ...
                    >
                      ...
                      {m.id === 'init' && (
                        ...
                        <div className="mt-3 pt-3 border-t flex flex-col gap-2">
                          ...
                          <button
                            onClick={() =>
                              sendMessage('Show my appointments')
                            }
                            className="flex items-center gap-2 text-[12px] bg-slate-50 p-2 rounded-lg hover:bg-teal-50 transition-colors"
                          >
                    ...
                    {m.role === 'assistant' && (
                      ...
                      <button
                        onClick={() => speak(m.content)}
                        className="text-gray-400 hover:text-teal-600 mb-2"
                      >
                        <Volume2 size={14}/>
                      </button>
            ...
            <div className="p-4 bg-white border-t">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => sendMessage()}
                  disabled={loading}
                  className="w-10 h-10 rounded-full bg-teal-600 text-white flex items-center justify-center disabled:opacity-50"
                >
                  <Send size={16}/>
                </button>
              ...
              <p className="text-[10px] text-center text-gray-400 mt-2">
                Voice & Chat Supported • AI Assistant
              </p>
